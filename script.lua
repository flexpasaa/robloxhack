...
local ConfigFileName = "Flex_Config_v650.json"
...
local ESP_Cache = {}
local UI_Elements = { Toggles = {}, Sliders = {}, StatusLabels = {} }
...
TabContainer.Position = UDim2.new(1, -290, 0, 0)
TabContainer.Size = UDim2.new(0, 270, 1, -2)
...
local function CreatePage(pageName)
    ...
    PageLeft.CanvasSize = UDim2.new(0, 0, 0, 500)
    PageLeft.Visible = false
    Instance.new("UIListLayout", PageLeft).Padding = UDim.new(0, 8)
    ...
    PageRight.CanvasSize = UDim2.new(0, 0, 0, 500)
    PageRight.Visible = false
    Instance.new("UIListLayout", PageRight).Padding = UDim.new(0, 8)
...
local function AddSlider(pageName, side, text, configKey, min, max)
    ...
    SliderBg.Position = UDim2.new(0, 2, 0, 22)
    ...
    SliderBtn.Position = UDim2.new(initPercent, -4, 0.5, -4)
    ...
    local function move()
        ...
        SliderBtn.Position = UDim2.new(percentage, -4, 0.5, -4)
...
local TpList = Instance.new("UIListLayout", TpScroll) TpList.Padding = UDim.new(0, 5)
...
local function UpdateTpMenu()
    ...
    TpScroll.CanvasSize = UDim2.new(0, 0, 0, count * 31)
end
Players.PlayerAdded:Connect(UpdateTpMenu) Players.PlayerRemoving:Connect(UpdateTpMenu)

AddToggle("Combat", "Left", "Aimbot", "Aimbot")
AddToggle("Combat", "Left", "Always Active", "AlwaysActive")
...
AddSlider("Misc", "Left", "Fly Speed", "FlySpeed", 10, 1000)   
...
AddButton("Misc", "Right", "Player Teleport Panel", function() UpdateTpMenu(); TpMenu.Visible = not TpMenu.Visible end)
...
local function GetClosestPlayerScreen()
    local closest, minDist = nil, math.huge
...
RunService.Stepped:Connect(function()
    if LocalPlayer.Character then
        ...
        if Config.Speed and not Config.Fly and LocalPlayer.Character:FindFirstChild("Humanoid") then
...
local function initESP()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and not ESP_Cache[p] then
            ESP_Cache[p] = createDrawings()
        end
    end
...
RunService:BindToRenderStep("Flex_Aimbot_Engine", Enum.RenderPriority.Camera.Value + 1, function(deltaTime)
    ...
    local aimTriggered = Config.Aimbot and (Config.AlwaysActive or UserInputService:IsMouseButtonPressed(Config.AimKey) or UserInputService:IsKeyDown(Config.AimKey))
    if aimTriggered and not Config.MenuOpen then
        currentTarget = GetClosestPlayerWorld()
        if currentTarget and currentTarget.Character then
            local part = currentTarget.Character:FindFirstChild("Head") or currentTarget.Character:FindFirstChild("UpperTorso") or currentTarget.Character:FindFirstChild("HumanoidRootPart")
            if part then
                local velocity = part.AssemblyLinearVelocity or part.Velocity or Vector3.new(0, 0, 0)
                local predictedPos = part.Position + (velocity * 0.045)
                local dist = (Camera.CFrame.Position - part.Position).Magnitude
                
                local smoothFactor = math.clamp(0.12 + (100 / dist), 0.1, 0.45)
...
RunService.RenderStepped:Connect(function(deltaTime)
    ...
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if Config.Fly then
            ...
            if UserInputService:IsKeyDown(Enum.KeyCode.W) then moveDir = moveDir + forwardVec leanPitch = -15 end
            if UserInputService:IsKeyDown(Enum.KeyCode.S) then moveDir = moveDir - forwardVec leanPitch = 15 end
            if UserInputService:IsKeyDown(Enum.KeyCode.A) then moveDir = moveDir - rightVec leanRoll = 20 end
            if UserInputService:IsKeyDown(Enum.KeyCode.D) then moveDir = moveDir + rightVec leanRoll = -20 end
            if UserInputService:IsKeyDown(Enum.KeyCode.Space) then moveDir = moveDir + Vector3.new(0, 1, 0) end
            if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then moveDir = moveDir - Vector3.new(0, 1, 0) end
            
            if moveDir.Magnitude > 0 then
                local targetPosition = rootPart.Position + (moveDir.Unit * (Config.FlySpeed * deltaTime))
                local baseCFrame = CFrame.new(targetPosition, targetPosition + forwardVec)
                ...
                rootPart.CFrame = CFrame.new(rootPart.Position, rootPart.Position + forwardVec) * flightRotation
            end
        else
    ...
    for player, esp in pairs(ESP_Cache) do
        ...
        if isAlive and root and head and Config.ESP then
            ...
            if headOnScreen or legOnScreen then
                ...
                local dynamicEspColor = isPlayerVisible and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
